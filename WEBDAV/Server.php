...
class HTTP_WebDAV_Server 
{
     ...
     * @var string 
     */
    var $http_auth_realm = "TYPO3 WebDAV";
     ...
     * @var string 
     */
    var $dav_powered_by = "Balisky Services";
    ...
    {
		    $this->tx_cbywebdav_devlog(4," ServeRequest server_array :".serialize($this->_SERVER) ,"pear_webdav","ServeRequest");
        ...
        $baseuri.="//".$this->_SERVER['HTTP_HOST'];
        $baseuri.='/'.$this->CFG->WEBDAVPREFIX;
        //$uri.= "//$this->_SERVER[HTTP_HOST]$this->_SERVER[SCRIPT_NAME]";
        $reluri=str_replace('/'.$this->CFG->WEBDAVPREFIX,'',$this->_SERVER["REQUEST_URI"]);
        $uri=$baseuri.$reluri;
        $this->relbaseuri='/'.$this->CFG->WEBDAVPREFIX;
        
        //$uri=$this->CFG->t3io->T3CleanFilePath($uri);
        //.....
        // ?
        
        $basepath='/';
        $path_info = empty($this->_SERVER["PATH_INFO"]) ? $reluri : $this->_SERVER["PATH_INFO"];        
        $this->base_uri=$baseuri;
 	      //$this->tx_cbywebdav_devlog(1," ServeRequest uri : $uri base_uri  : $this->base_uri" ,"pear_webdav","ServeRequest");
 	      //if ($path_info!=str_replace($uri)
        $this->uri=$uri; //.$path_info;
	      //$this->tx_cbywebdav_devlog(1," ServeRequest t->uri : $this->uri base_uri  : $this->base_uri pathinfo : $path_info" ,"pear_webdav","ServeRequest");
		    //$this->tx_cbywebdav_devlog(1," ServeRequest uri : $uri $ath_info : $path_info method :".$this->_SERVER['REQUEST_METHOD'] ,"pear_webdav","ServeRequest");
				$this->tx_cbywebdav_devlog(1,"= ServeRequest method : ".$this->_SERVER["REQUEST_METHOD"]." uri :$this->uri base_uri  : $this->base_uri pathinfo : $path_info" ,"pear_webdav",'ServeRequest');
        ...
        if (!strlen($this->path)) {
            if ($this->_SERVER["REQUEST_METHOD"] == "GET") {
                ...
                $this->tx_cbywebdav_devlog(1," ServeRequest redirect : $this->base_uri" ,"pear_webdav","ServeRequest");
                header("Location: ". $this->_slashify($this->base_uri));
                return;
            } else {
        ...
        if ( (   !(($this->_SERVER['REQUEST_METHOD'] == 'OPTIONS') && ($this->path == $basepath)))
             && (!$this->_check_auth())) {
    ...
    */
    ...
    /* abstract 
     ...
     {
     	  $this->tx_cbywebdav_devlog(1," checklock:".$status ,"pear_webdav");
				return array();
     // dummy entry for PHPDoc
     } */
    ...
    {
  			$this->tx_cbywebdav_devlog(1," http_OPTIONS:".$status ,"pear_webdav","http_OPTIONS");
    ...
    function http_PROPFIND() 
    {
        ...
        foreach ($files["files"] as $file) {
            ...
            //$href = $this->_mergePathes($this->_SERVER['SCRIPT_NAME'], $path);
    			$this->tx_cbywebdav_devlog(2,"=== patch  ".$this->_SERVER["SCRIPT_NAME"]." path $path relbaseuri $this->relbaseuri","cby_webdav","patch");
            $href = $this->_mergePathes($this->relbaseuri, $path);
    ...
    function http_PROPPATCH() 
    {
        if ($this->_check_lock_status($this->path)) {
           ...
           // echo "  <D:href>".$this->_urlencode($this->_mergePathes($this->_SERVER["SCRIPT_NAME"], $this->path))."</D:href>\n";
    			$this->tx_cbywebdav_devlog(2,"=== patch  ".$this->_SERVER["SCRIPT_NAME"]." path $path $this->relbaseuri $relbaseuri","cby_webdav","patch");
           echo "  <D:href>".$this->_urlencode($this->_mergePathes($this->relbaseuri, $this->path))."</D:href>\n";
    ...
    function http_HEAD() 
    {
        ...
        if (method_exists($this, "HEAD")) {
            ...
            $status = $this->head($options);
  					$this->tx_cbywebdav_devlog(1," http_HEAD:".$status ,"pear_webdav",'http_HEAD');
      } else if (method_exists($this, "GET")) {
            ob_start();
            $status = $this->GET($options);
  					$this->tx_cbywebdav_devlog(1," http_GET:".$status ,"pear_webdav",'http_HEAD');
             if (!isset($options['size'])) {
    ...
    function http_PUT() 
    {
        if ($this->_check_lock_status($this->path)) {
            ...
            */ 
            ...
            $stat2 = $this->AFTERPUT($options);
		        // If file is put in a directory that is locked by us , we must il lock also ...
		        $parent = $this->_slashify(dirname($this->path));
		 				//$parentLock=!$this->_check_lock_status($parent);
		 				$parentLock=$this->checkLock($parent);
		 	 			$this->tx_cbywebdav_devlog(1,"=== http_PUT lock ?, webdav path : $virtualFilePath , parent : $parent , parentLock $parentLock","cby_webdav","PUT");
		 
		 	 			if (is_array($parentLock) && count($parentLock)) {
		 	 				$this->tx_cbywebdav_devlog(1,"=== http_PUT PUT ?, lock options :".serialize($lockoptions),"cby_webdav","PUT");
							// refresh lock
            	...
            	$options['owner']     = $parentLock['owner'];
            	$options['scope']     = $parentLock['scope'];
            	$options['type']      = $parentLock['type'];
							$ret=$this->LOCK($options);
		 	 			}
						$this->tx_cbywebdav_devlog(1," http_AFTERPUT : ".$stat2,"pear_webdav",'http_PUT');
						$this->tx_cbywebdav_devlog(1," http_PUT : ".$stat,"pear_webdav",'http_PUT');
            $this->http_status($stat);
        } else {
            $this->http_status("423 Locked");
        }
    ...
    function http_LOCK() 
    {
				$this->tx_cbywebdav_devlog(1," start","pear_webdav",'http_LOCK');
        $options         = Array();
        $options["path"] = $this->path;
        ...
        if (empty($this->_SERVER['CONTENT_LENGTH']) && !empty($this->_SERVER['HTTP_IF'])) {
            ...
            if (!$this->_check_lock_status($this->path)) {
  							$this->tx_cbywebdav_devlog(1," locked !","pear_webdav",'http_LOCK');
              	$this->http_status("423 Locked");
                return;
            }
    ...
    {
				$this->tx_cbywebdav_devlog(1," _copymove : ".$what,"pear_webdav",'_copymove');
        $options         = Array();
        $options["path"] = $this->path;
        ...
        if (isset($this->_SERVER["HTTP_DEPTH"])) {
            ...
            $options["depth"] = "infinity";
				...
				// We import variables from url ... Wicked !
        ...
        $http_header_host = preg_replace("/:80$/", "", $this->_SERVER["HTTP_HOST"]);
				$this->tx_cbywebdav_devlog(1," _copymove scheme $scheme host $host port $port user $user pass $pass path $path query $query fragment $fragment script ".$this->_SERVER["SCRIPT_NAME"],"pear_webdav",'_copymove');

        if ($http_host == $http_header_host && !strncmp($this->_SERVER["SCRIPT_NAME"], $path,strlen($this->_SERVER["SCRIPT_NAME"])) ) {

            $options["dest"] = substr($path, strlen($this->_SERVER["SCRIPT_NAME"]));
            if (!$this->_check_lock_status($options["dest"])) {
                $this->http_status("423 Locked");
 								$this->tx_cbywebdav_devlog(1," _copymove $what options : ".serialize($options),"pear_webdav",'_copymove');
               return;
            }
        } elseif ($http_host == $http_header_host && !strncmp('/'.$this->CFG->WEBDAVPREFIX, $path,strlen($this->CFG->WEBDAVPREFIX)+1) ) {

            $options["dest"] = substr($path, strlen($this->CFG->WEBDAVPREFIX)+1);
            if (!$this->_check_lock_status($options["dest"])) {
                $this->http_status("423 Locked");
 								$this->tx_cbywebdav_devlog(1," _copymove $what options : ".serialize($options),"pear_webdav",'_copymove');
               return;
    ...
    function _allow() 
    {
 				$this->tx_cbywebdav_devlog(1,"******* _allow start","pear_webdav","_allow");
       // OPTIONS is always there
        ...
        foreach (get_class_methods($this) as $method) {
            if (!strncmp("http_", $method, 5)) {
                $method = strtoupper(substr($method, 5));
 								$this->tx_cbywebdav_devlog(1,"******* _allow:".$method ,"pear_webdav","_allow");
        ...
        if (!method_exists($this, "checklock")) {
            ...
            unset($allow["UNLOCK"]);
 				...
 				$this->tx_cbywebdav_devlog(1,"******* _allow end:".serialize($allow) ,"pear_webdav","_allow");
        return $allow;
    }
    ...
    {
 			$this->tx_cbywebdav_devlog(1," in path :$path, exclusive : $exclusive_only","pear_webdav",'_check_lock_status');
       ...
       if (method_exists($this, "checkLock")) {
            ...
            if (is_array($lock) && count($lock)) {
                // FIXME doesn't check uri restrictions yet
        				$this->tx_cbywebdav_devlog(1," HTTP_IF :".$this->_SERVER["HTTP_IF"]." lock :".serialize($lock),"pear_webdav",'_check_lock_status');

                if (!isset($this->_SERVER["HTTP_IF"]) || !strstr($this->_SERVER["HTTP_IF"], $lock["token"])) {
        						$this->tx_cbywebdav_devlog(1," HTTP_IF 2:".$this->_SERVER["HTTP_IF"]." lock :".serialize($lock)." EO ".(!$exclusive_only),"pear_webdav",'_check_lock_status');
                    if (!$exclusive_only || ($lock["scope"] !== "shared"))   $ret= false;
                }
            }
        }
        $this->tx_cbywebdav_devlog(1," out: $ret ,lock :".serialize($lock),"pear_webdav",'_check_lock_status');
        return $ret;
    }
    ...
    function http_status($status) 
    {
        ...
        $this->_http_status = $status;
 				$this->tx_cbywebdav_devlog(1,"******* :".$status ,"pear_webdav",'http_status');
